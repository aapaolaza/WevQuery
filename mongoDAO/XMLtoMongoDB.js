...
if (require.main === module) {
  ...
  if (argv.strictMode) { mapReduceVars.isQueryStrict = true; }
  console.log(`StrictMode is ${mapReduceVars.isQueryStrict}`);

  // Start the xml Loading
  loadXml(argv.file, (err, data) => {
    ...
    prepareXml(xmlDoc, mapReduceVars, (err, data) => {
      // notify user of the error, or the result of the query
...
function getXmlQueryData(queryTitle, callback) {
  ...
  constants.connectAndValidateNodeJs((connectErr, db) => {
    if (connectErr) return console.error(`getXmlQueryData() ERROR connecting to DB${connectErr}`);
    console.log('getXmlQueryData() Successfully connected to DB');

    // Does the corresponding collection exist?
    db.listCollections({ name: collectionTitle }).toArray((listCollErr, items) => {
      if (listCollErr) return console.error(`getXmlQueryData() ERROR REQUESTING COLLECTION ${listCollErr}`);
      if (items.length === 1) {
        // Collection exists, query its elements
        // var cursor =
        db.collection(collectionTitle).find({ 'value.xmlQueryCounter': { $gt: 0 } }).toArray((findErr, documents) => {
...
function getXmlQueryDataByCollection(queryCollName, callback) {
  constants.connectAndValidateNodeJs((connectErr, db) => {
    if (connectErr) return console.error(`getTempXmlQueryData() ERROR connecting to DB ${connectErr}`);
    console.log('getTempXmlQueryData() Successfully connected to DB');

    // Does the corresponding collection exist?
    db.listCollections({ name: queryCollName }).toArray((listCollErr, items) => {
      if (listCollErr) return console.error(`getTempXmlQueryData() ERROR REQUESTING COLLECTION ${listCollErr}`);
      if (items.length === 1) {
        // Collection exists, query its elements
        // var cursor =
        db.collection(queryCollName).find({ 'value.xmlQueryCounter': { $gt: 0 } }).toArray((err, documents) => {
          console.log('printing results');
          console.log(`Returning ${documents.length} items`);
...
function deleteResultCollection(queryTitle, callback) {
  constants.connectAndValidateNodeJs((connectErr, db) => {
    if (connectErr) return console.error(`deleteResultCollection() ERROR connecting to DB${connectErr}`);
    db.collection(queryCollectionPrefix + queryTitle).drop((dropErr, result) => {
      if (dropErr) {
        console.log(dropErr);
      }
      callback(null);
...
function deleteTempResultCollection(queryCollName, callback) {
  ...
  if (!queryCollName.indexOf(queryCollectionTempPrefix) === 0) {
    return console.error(`collection name ${queryCollName} is not a temporal collection`);
  }

  constants.connectAndValidateNodeJs((connectErr, db) => {
    if (connectErr) return console.error(`deleteTempResultCollection() ERROR connecting to DB ${connectErr}`);
    db.collection(queryCollName).drop((dropErr, result) => {
      if (dropErr) {
        console.log(dropErr);
      }
      callback(null);
...
function deleteAllTempResultCollections() {
  constants.connectAndValidateNodeJs((connectErr, db) => {
    if (connectErr) return console.error(`deleteAllTempResultCollections() ERROR connecting to DB ${connectErr}`);

    db.listCollections().toArray((listCollErr, collInfos) => {
      // collInfos is an array of collection info objects that look like:
      // { name: 'test', options: {} }
      collInfos.forEach((collObject, index) => {
        if (collObject.name.indexOf(queryCollectionTempPrefix) === 0) {
          ...
          db.collection(collObject.name).drop((dropErr, result) => {
            if (dropErr) {
              console.log(dropErr);
            }
          });
...
function runXmlQuery(title, xmlQuery, queryOptions, endCallback, launchedCallback) {
  ...
  const xmlDoc = new dom().parseFromString(xmlQuery);
...
/**
 * Runs the provided xmlQuery, and stores it in a temporary collection.
 * Accessible from the WevQueryServer
 * @param {string} title of the query, the results will be stored in a collection of the same name
 * @param {string} xmlQuery string containing the xml query to run
 * @param {object} queryOptions various options to be applied to the query.
 * @param {runXmlQueryCallback} callback to call when completed
...
function executeXmlMapReduce(xmlQuery, xmlDoc, mapReduceVars, endCallback, launchedCallback) {
  ...
  const eventCollection = mapReduceVars.db.collection(constants.eventCollection);
  // The xml needs to be processed, and transformed into JavaScript objects that MapReduce can process
  const mapReduceXMLQueryObject = parseXMLToMapReduceObject(xmlDoc);
  ...
  const value = eventCollection.mapReduce(
    ...
    (err, results, stats) => { // stats provided by verbose
      console.log('executeXmlMapReduce() end');

      if (err) {
        mongoLog.logMessage(
          'optime', 'mapReduceScript',
...
function parseXMLToMapReduceObject(xmlDoc) {
  ...
  while (areEventsLeft) {
    ...
    if (xpath.select(`boolean(//event[@pre='${currentID}'])`, xmlDoc)) {
      const eventQueryObject = new Object();
      ...
      const context = xpath.select(`//event[@pre='${currentID}']/context`, xmlDoc);
  ...
  const tempRestrNodeList = xpath.select('//temporalconstraint', xmlDoc);
  tempRestrNodeList.forEach((tempRestrNode, index) => {
    tempRestrObject = new Object();
    tempRestrObject.type = tempRestrNode.getAttribute('type');

    if (tempRestrNode.getAttribute('unit') == 'sec') { tempRestrObject.value = tempRestrNode.getAttribute('value') * 1000; } else if (tempRestrNode.getAttribute('unit') == 'min') { tempRestrObject.value = tempRestrNode.getAttribute('value') * 60 * 1000; } else { interruptExecution('parseXMLToMapReduceObject(): ERROR WITH THE UNIT VALUE OF TEMPORAL CONSTRAINT'); }

    // retrieve the event Ids for the temp constraints, and check the constructed table for the corresponding indexes
    const eventRef = tempRestrNode.getElementsByTagName('eventref');
...
function retrieveQueriedFields(mapReduceXMLQueryObject) {
  ...
  const requiredFieldList = ['_id', 'sd', 'sid', 'timestampms', 'event', constants.episodeField];

  // Additional fields to support the MapReduce query
  mapReduceXMLQueryObject.eventList.forEach((eventObject, index) => {
    // CAUTION the context is not actually a list, but an object that contains a list of type and value
    eventObject.context.typeList.forEach((contextTypeField, index) => {
      if (requiredFieldList.indexOf(contextTypeField) === -1) { requiredFieldList.push(contextTypeField); }
    });
  });
...
function mapFunction() {
  ...
  */
  ...
  requiredFieldList.forEach((requiredField, index) => {
    // is the requiredField a nested field? if so the nesting needs to be tackled.
    if (requiredField.indexOf('.') > -1) {
      const mainField = requiredField.split('.')[0];
      let nestedField = requiredField.split('.')[1];

      // Only emit this field if the event contains it
      if (eventToEmit[mainField]) {
        let contentToEmit = eventToEmit[mainField][nestedField];
    ...
    } else { emitData[requiredField] = eventToEmit[requiredField]; }
  });


  // emit({sid:this.sid, sessionstartms:this.sessionstartms, url:this.url, urlSessionCounter:this.urlSessionCounter},
  emit(
    { sid: this.sid, url: this.url, episodeCount: this[episodeField] },
    {
      episodeEvents:
        [
          emitData,
...
function reduceFunction(key, values) {
  const reduced = { episodeEvents: [] };
  for (const i in values) {
    const inter = values[i];
    for (const j in inter.episodeEvents) {
...
function skinnyReduceFunction(key, values) {
  ...
  for (const i in values) {
    ...
    for (const j in inter.episodeEvents) {
      ...
      for (const fieldIndex in reduced.episodeEvents[j]) {
 ...
 */
  ...
  XmlQuery.prototype.processEvent = function (currentEvent) {
    ...
    const xmlQuery = this;
    this.xmlQueryCandidatesList.forEach((xmlQueryCandidate, index) => {
      const indexToMatch = xmlQueryCandidate.length;
    ...
    if (xmlQueryObject.eventList[0].nameList.indexOf(currentEvent.event) > -1
      ...
      if (xmlQueryObject.eventList.length == 1) { xmlQuery.xmlQueryList.push(candidateObject); } else { this.xmlQueryCandidatesList.push(candidateObject); }
    }
  };
  ...
  function matchTemporalConstraintList(xmlQueryCandidate, tempConstraintList) {
    ...
    for (let index = 0; index < tempConstraintList.length; index++) {
      const tempConstraint = tempConstraintList[index];
      ...
      if (tempConstraint.eventRef1 < xmlQueryCandidate.length &&
        ...
        const event1 = xmlQueryCandidate[tempConstraint.eventRef1];
        const event2 = xmlQueryCandidate[tempConstraint.eventRef2];
        const timeDistance = Math.abs(event1.timestampms - event2.timestampms);

        if (tempConstraint.type == 'within' && timeDistance > tempConstraint.value) { return (0); } else if (tempConstraint.type == 'between' && timeDistance < tempConstraint.value) { return (0); }
      }
      // });
    }
  ...
  function matchContextInfoComplex(currentEvent, contextInfo) {
    ...
    for (i = 0; i < contextInfo.typeList.length; i++) {
      ...
      if (fieldName.indexOf('.') > -1) {
        ...
        let nestedField = fieldName.split('.')[1];
        let contentToCompare = currentEvent[mainField][nestedField];
        ...
        if (contentToCompare != contextInfo.valueList[i]) { return false; }
      } else if (currentEvent[fieldName] != contextInfo.valueList[i]) { return false; }
    }
    return true;
  }
  ...
  function matchContextInfo(currentEvent, contextInfo) {
    for (i = 0; i < contextInfo.typeList.length; i++) {
      if (currentEvent[contextInfo.typeList[i]] != contextInfo.valueList[i]) { return false; }
    }
    return true;
  }
  ...
  // /////////////////////////////////////////START OF FUNCTION//////////////////////////
  // //////////////////////////////////////////////////////////////////////////////////////////////////////
  const valuesArray = reduceOutput.episodeEvents;
...
function feedQueryResultsByTitle(queryTitle, callback) {
  const startTimems = new Date();
  const queryCollName = queryCollectionPrefix + queryTitle;
...
function updateXmlQueryDocument(docElem, db, queryCollName, callback) {
  ...
  for (const xmlQueryIndex in docElem.value.xmlQuery) {
    xmlQueryResult = docElem.value.xmlQuery[xmlQueryIndex];
    xmlQueryResult.forEach((eventInQuery, eventIndexInResult) => {
      resultTotal++;
    });
  }
  ...
  for (const xmlQueryIndex in docElem.value.xmlQuery) {
    ...
    xmlQueryResult.forEach((eventInQuery, eventIndexInResult) => {
      ...
      db.collection(constants.eventCollection).find({ _id: eventInQuery._id }).toArray((err, eventFullInfo) => {
        ...
        const xmlQueryEventUpdatedValue = {};
        ...
        */
        db.collection(queryCollName).updateOne(
          xmlQueryEventIndex,
          { $set: xmlQueryEventUpdatedValue }, (err, doc) => {
            resultCount++;
            if (err)
              callback(err);
            else {
